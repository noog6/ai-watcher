...
class ICM20948Sensor(object):
  ...
  def __init__(self,address=I2C_ADD_ICM20948):
    if not ICM20948Sensor._instance:
      ...
      raise Exception("You cannot create another ICM20948Sensor class")
  ...
  def calc_pitch_degrees(self):
    self.pitch = math.asin( -2 * self.q1 * self.q3 + 2 * self.q0 * self.q2)* 57.3
    return self.pitch

  def calc_roll_degrees(self):
    self.roll  = math.atan2( 2 * self.q2 * self.q3 + 2 * self.q0 * self.q1, -2 * self.q1 * self.q1 - 2 * self.q2 * self.q2 + 1)* 57.3
    return self.roll

  def calc_yaw_degrees(self):
    self.yaw   = math.atan2(-2 * self.q1 * self.q2 - 2 * self.q0 * self.q3,  2 * self.q2 * self.q2 + 2 * self.q3 * self.q3 - 1) * 57.3
    return self.yaw  
